  ...
  room
    ...
    .forEach((p) => {
      ...
      if (p.team != forTeam) {
        ...
        if (
          Math.sign(pos.x) * p.position.x > 1176 &&
          p.position.y > -448 &&
          p.position.y < 448
        ) {
          room.setPlayerDiscProperties(p.id, { x: Math.sign(pos.x) * 1140 });
        }
      }
    });
...
const throwIn = async (
  ...
  if (forTeam == 1) {
    if (pos.y < 0) {
      // show top red line
      room.setDiscProperties(17, { x: 1609 });
      // hide top blue line
      room.setDiscProperties(19, { x: -1609 });
    } else {
      // show bottom red line
      room.setDiscProperties(21, { x: 1609 });
      // hide bottom blue line
      room.setDiscProperties(23, { x: -1609 });
    }
  } else {
    if (pos.y < 0) {
      // show top blue line
