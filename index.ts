...
const roomBuilder = async (HBInit: Headless, args: RoomConfigObject) => {
  console.log("iniciando a sala...");
  room = HBInit(args);
  db = await initDb();
  const rs7Stadium = fs.readFileSync("./maps/rs7.hbs", {
    encoding: "utf8",
    flag: "r",
  });
  room.setCustomStadium(rs7Stadium);
  room.setTimeLimit(0);
  ...
  if (game) {
    ...
    // Atualiza previousTouch antes de mudar o lastTouch
    if (game.lastTouch) {
      game.previousTouch = {
        byPlayer: game.lastTouch.byPlayer,
        x: game.lastTouch.x,
        y: game.lastTouch.y
      };
    }

    // Atualiza lastTouch com dados atuais
    const ball = room.getDiscProperties(0);
    if (ball) {
      game.lastTouch = {
        byPlayer: pp,
